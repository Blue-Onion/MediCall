      ...
      <div className="max-w-2xl mx-auto">
        <Card className={ "border-emerald-900/50 max-w-xl hover:border-emerald-700/40 cursor-pointer transition-all duration-300 ease-in-out"}>
          <CardHeader className={"text-center"}>
            <div className={`mx-auto p-4 ${isRejected?"text-red-900/20":"text-amber-900/20"}`}>
              {isRejected?(<XCircle className="h-8 w-8 text-red-400"/>):
              (
                <ClipboardCheck className="h-8 w-8 text-amber-400"/>
              )
              }
            </div>
            <CardTitle className={"text-2xl font-bold"}>{isRejected?"Verification is declined":"Verification in Progress"}</CardTitle>
            <CardDescription>{isRejected?"Unfortunately you application needs to revised":"Thank you for submitting your information"}</CardDescription>
